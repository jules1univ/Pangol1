...
public final class AxisSettingsPanel extends SettingSectionPanel {
    ...
    public AxisSettingsPanel() {
        super(Lang.get("report.settings.chart.axis"));
        build();
    }
    ...
    private void build() {
        ...
        addRow(new SettingRowPanel(Lang.get("report.settings.chart.tick_count"),
                tickCountSpinner));

        scaleCombo = new JComboBox<>(new String[] {
                Lang.get("report.settings.chart.linear"),
                Lang.get("report.settings.chart.logarithmic"),
                Lang.get("report.settings.chart.sqrt")
        });
        addRow(new SettingRowPanel(Lang.get("report.settings.chart.scale"), scaleCombo));

        showXAxisCheck = new JCheckBox();
        showXAxisCheck.setSelected(true);
        addRow(new SettingRowPanel(Lang.get("report.settings.chart.show_x_axis"),
                showXAxisCheck));
        ...
        addRow(new SettingRowPanel(Lang.get("report.settings.chart.show_y_axis"),
                showYAxisCheck));

        yAxisLabelField = new JTextField(Lang.get("report.settings.chart.default_labely"));
        addRow(new SettingRowPanel(Lang.get("report.settings.chart.y_label"),
                yAxisLabelField));
